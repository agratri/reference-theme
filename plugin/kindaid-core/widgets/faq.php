...
class Kindaid_Faq extends \Elementor\Widget_Base {
	...
	public function get_title(): string {
		return esc_html__( 'Faq', 'kindaid-core' );
	}
	...
	protected function register_controls_section(){
		...
		$this->add_control(
			...
			[
				...
				'default' => [
					[
						'title' => esc_html__( 'Title Here', 'kindaid-core' ),
						'content' => esc_html__( 'Default Content Here one', 'kindaid-core' ),
					],
					[
						'title' => esc_html__( 'Title Here', 'kindaid-core' ),
						'content' => esc_html__( 'Default Content Here two', 'kindaid-core' ),
					],
				],
				'title_field' => '{{{ title }}}',
			]
	...
	protected function render(): void {
		...
		<div class="tp-faq ml-15 wow fadeInUp" data-wow-duration=".9s" data-wow-delay=".3s">
			<div class="accordion" id="accordionExample">
				<?php foreach( $settings['list'] as $key => $item ) : 
					$active = ($key == 0) ? 'active'	: '';
					$show = ($key == 0) ? 'show'	: '';
					$num = ( $key + 1 < 10 ) ? '0' . ( $key + 1 ) : ( $key + 1 );
				?>
				<div class="tp-faq-item <?php echo esc_attr($active); ?>">
					<h2 class="accordion-header">
						<button class="tp-faq-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne-<?php echo esc_attr($key); ?>" aria-expanded="true" aria-controls="collapseOne-<?php echo esc_attr($key); ?>">
