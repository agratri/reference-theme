...
class KindAid_Walker_Nav_Menu extends Walker {
	...
	/**
	 * Fallback output when no menu is assigned to the requested location.
	 *
	 * Referenced as 'fallback_cb' by kindaid_main_menu() and kindaid_footer_menu().
	 * Without this method wp_nav_menu() finds the callback uncallable and prints
	 * nothing at all, which looks like a broken theme rather than an unset menu.
	 *
	 * Only users who can manage menus see the prompt; visitors see nothing, so a
	 * live site is never disfigured by it.
	 *
	...
	public static function fallback( $args = array() ) {
		...
		printf(
			...
			esc_html(
				sprintf(
					/* translators: %s: menu location slug, e.g. main-menu */
					__( 'Assign a menu to "%s"', 'kindaid' ),
					$location
				)
			)
		);
