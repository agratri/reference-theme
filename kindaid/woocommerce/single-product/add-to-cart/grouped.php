...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								if ( $grouped_product_child->is_on_sale() ) {
									printf(
										...
										esc_html__( 'Buy one of %1$s on sale for %2$s, original price was %3$s', 'kindaid' ),
									...
									printf(
										...
										esc_html__( 'Buy one of %1$s for %2$s', 'kindaid' ),
										esc_html( $grouped_product_child->get_name() ),
										esc_html( wp_strip_all_tags( wc_price( $grouped_product_child->get_price() ) ) )
									);
								}
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<div class="tp-product-details-add-to-cart mb-15 mr-10">
			<button type="submit" class="tp-product-details-add-to-cart-btn w-100 single_add_to_cart_button alt<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
		</div>
